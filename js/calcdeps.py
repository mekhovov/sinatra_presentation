...
req_all_regex = re.compile('goog\.requireAll\s*\(\s*([^\)]+)\s*\)')
prov_regex = re.compile('goog\.provide\s*\(\s*[\'\"]([^\)]+)[\'\"]\s*\)')
prov_all_regex = re.compile('goog\.provideAll\s*\(\s*([^\)]+)\s*\)')
  ...
  for filename in files:
    ...
    try:
      for line in file_handle:
        ...
        if re.match(req_all_regex, line):
          dep.requires = re.search(req_all_regex, line).group(1)
        if re.match(prov_regex, line):
          dep.provides.append(re.search(prov_regex, line).group(1))
        if re.match(prov_all_regex, line):
          dep.provides = re.search(prov_all_regex, line).group(1)
    finally:
      file_handle.close()
    result.append(dep)
